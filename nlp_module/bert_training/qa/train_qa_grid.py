...
def compute_ndcg(model, data):
    """
    Since each query has one correct answer, the DCG is 1/log2(rank+1)
...
class ComprehensiveEvaluator(SentenceEvaluator):
    ...
    def __call__(self, model, output_path=None, epoch=-1, steps=-1):
        ...
        return mrr_value  # Choose one metric for early stopping if desired
...
# Create examples using only the English questions and answers
examples = [InputExample(texts=[qa["question"], qa["answer"]]) for qa in qa_data]
...
for i, params in enumerate(param_grid):
    ...
    if loss_fn_name == "MultipleNegativesRankingLoss":
        ...
        # Note: CosineSimilarityLoss requires labels; for demonstration, we use a simple form.
        train_loss = losses.CosineSimilarityLoss(model=model)
    else:
        # Default to MultipleNegativesRankingLoss if unknown
        train_loss = losses.MultipleNegativesRankingLoss(model)
...
if best_model is not None:
    best_model.save("modelo_QA_xlm-roberta_best_v2")
